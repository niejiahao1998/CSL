...
import pdb
import os
### We import cv2 lib for concatenating rgb abd v channels  ### date-2022/AUG/03
import cv2

data_path = '/home/reid/datasets/SYSU_MM01/SYSU-MM01/'
...
def read_imgs(train_image):
    ...
    for img_path in train_image:
        ...
        ### We add these lines for concatenating rgb abd v channels  ### date-2022/AUG/03
        ...
        h, s, v = cv2.split(hsv)
        new_v = v[:,:,np.newaxis]
        new_img = np.concatenate((img, new_v), axis=2)
        pix_array = np.array(new_img)
