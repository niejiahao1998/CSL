...
from data_loader import SYSUData, RegDBData, TestData, NTUData
...
if dataset == 'sysu':
    ...
    test_mode = [1, 2]  # IR to visible
    # test_mode = [2, 1]  # visible to IR
    ...
    log_path = args.log_path + 'ntu_log/'
...
### We add this line for saving different logs and models for different methods ###date-2022/AUG/09
...
print('==> Loading data..')
# Data loading code
normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) # avg: mean=0.449, std=0.226
...
if dataset == 'sysu':
    # training set
    trainset = SYSUData(data_dir=data_path, transform=transform_train, height=args.img_h, width=args.img_w)
    ...
    gall_img, gall_label, gall_cam = process_gallery_sysu(data_path, mode=args.mode, trial=0, ir2rgb=test_mode[0])

elif dataset == 'regdb':
    # training set

    trainset = RegDBData(data_path, args.trial, transform=transform_train, height=args.img_h, width=args.img_w)
    # generate the idx of each person identity
    color_pos, thermal_pos = GenIdx(trainset.train_color_label, trainset.train_thermal_label)
    ...
    if test_mode[0] == 1:
        ...
        query_modal = 'visible'
        gall_modal = 'thermal'
    query_img, query_label = process_test_regdb(data_path, trial=args.trial, modal=query_modal)
    gall_img, gall_label = process_test_regdb(data_path, trial=args.trial, modal=gall_modal)

elif dataset == 'ntu':
    # training set
    trainset = NTUData(data_path, args.trial, transform=transform_train, height=args.img_h, width=args.img_w)
    color_pos, thermal_pos = GenIdx(trainset.train_color_label, trainset.train_thermal_label)
...
if args.method == 'agw':
    ...
    if args.dataset == 'regdb':
        criterion_tri = TripletLoss_WRT() # for RegDB
else:
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        ### We modify this model for 1*1 conv
        feat, out0, = net(input10, input11, input2)
